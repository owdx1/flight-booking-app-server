...
router.post('/login' , async (req , res) => {
  ...
  try {
    ...
    if(!existingFirm) {
      return res.status(400).json({message:'Bu emaile kayıtlı firma bulunamadı.'})
    }
    ...
    const firmToken =  jwt.sign(payload, process.env.FIRM_TOKEN_SECRET, {expiresIn:"1hr"});
    const firmTokenRefresh =  jwt.sign(payload , process.env.FIRM_TOKEN_REFRESH);
    const existingRefreshToken =  await Refresh.findOne({firm_id:existingFirm.id})
    if(existingRefreshToken) {
      await Refresh.deleteOne({firm_id:existingFirm.id})
    }
    const newRefreshToken = new Refresh({token:firmTokenRefresh , firm_id:existingFirm.id})
    await newRefreshToken.save();
    return res.status(200).json({message:'Başarıyla giriş yapıldı!' , firmToken:firmToken})
...
router.post('/register' , async (req, res) =>{
  ...
  try {
    ...
    const newFirm = new Firm({email:email ,phoneNumber:phoneNumber ,name:name, password:bcryptPassword})
    await newFirm.save();
    console.log(newFirm);
...
router.get('/dashboard', firmTokenValidator, firmTokenRefreshValidator, async (req, res) => {
